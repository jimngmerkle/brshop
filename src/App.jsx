  ...
  const checkOut = async (cartItems) => {
    if (cartItems.length <= 0) {
      ...
      if (confirmOrder) {
        ...
        // Send purchase data to BR, without waiting for response
        exponea.track('purchase', { purchase }).catch(console.error);
  
        // Clear the cart immediately after triggering the purchase event
        setCartItems([]);
